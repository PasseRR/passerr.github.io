...
1. 笔者开发时基于mybatis-plus的[3.4.3.2](https://github.com/baomidou/mybatis-plus/tree/v3.4.3.2){:target="_blank"}
2. mp本身提供了一个[DataPermissionInterceptor](https://github.com/baomidou/mybatis-plus/blob/v3.4.3.2/mybatis-plus-extension/src/main/java/com/baomidou/mybatisplus/extension/plugins/inner/DataPermissionInterceptor.java){:target="_blank"}数据权限拦截器
    ...
    ```
3. 由源码可知，mp集成jsqlparser(继承JsqlParserSupport实现)运行时根据条件动态追加过滤条件
...
**PS**：在结合[PageHelper](https://github.com/pagehelper/Mybatis-PageHelper){:target="_blank"}做带有数据权限的分页时遇到过一个问题，
